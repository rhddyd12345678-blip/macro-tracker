...
id_verified   source_id 를 실제로 확인했는지. N 이면 첫 ETL 실행 전 확인 필요
"""

import csv
from signs import resolve_sign, resolve_weight, needs_review

# (id, name_ko, country, category, source, source_id, freq, transform,
#  target_cb, use_as, id_verified)
INDICATORS = [
    # ============================================================ 미국 — 물가
...
HEADER = ["id", "name_ko", "country", "category", "source", "source_id", "freq",
          "transform", "target_cb", "use_as", "hawkish_sign", "weight",
          "sign_note", "review_flag", "id_verified"]


def build():
    rows, seen = [], set()
    for (iid, name, country, cat, src, sid, freq, tf, cb, use_as, verified) in INDICATORS:
        ...
        rows.append([iid, name, country, cat, src, sid, freq, tf, cb, use_as,
...
if __name__ == "__main__":
    ...
    print("용도별:", dict(Counter(r[9] for r in rows)))
    print("검토필요:", sum(1 for r in rows if r[13]))
    print("id확인필요:", sum(1 for r in rows if r[14] == "N"))
